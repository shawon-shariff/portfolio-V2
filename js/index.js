$(document).ready(function(){

	// loader
	paceOptions = {
		ajax: true,
		document: true,
		eventLag: false
		};

		Pace.on('done', function() {
		$('.p').delay(500).animate({top: '30%', opacity: '0'}, 3000, $.bez([0.19,1,0.22,1]));

		$('#preloader').delay(500).animate({top: '-100%'}, 2000, $.bez([0.19,1,0.22,1]));
	
		TweenMax.from(".title", 2, {
			 delay: 1.8,
				  y: 10,
				  opacity: 0,
				  ease: Expo.easeInOut
			})
	   });
	$('.nav-icon1').click(function(){
		$('.nav-icon1').addClass('active')
		$('.nav-icon2').removeClass('active')
		$('.nav-icon3').removeClass('active')
		$('.nav-icon4').removeClass('active')
		$('.nav-icon5').removeClass('active')
		$('.nav-icon6').removeClass('active')
	});
	$('.nav-icon2').click(function(){
		$('.nav-icon1').removeClass('active')
		$('.nav-icon2').addClass('active')
		$('.nav-icon3').removeClass('active')
		$('.nav-icon4').removeClass('active')
		$('.nav-icon5').removeClass('active')
		$('.nav-icon6').removeClass('active')
	});
	$('.nav-icon3').click(function(){
		$('.nav-icon1').removeClass('active')
		$('.nav-icon2').removeClass('active')
		$('.nav-icon3').addClass('active')
		$('.nav-icon4').removeClass('active')
		$('.nav-icon5').removeClass('active')
		$('.nav-icon6').removeClass('active')
	});
	$('.nav-icon4').click(function(){
		$('.nav-icon1').removeClass('active')
		$('.nav-icon2').removeClass('active')
		$('.nav-icon3').removeClass('active')
		$('.nav-icon4').addClass('active')
		$('.nav-icon5').removeClass('active')
		$('.nav-icon6').removeClass('active')
	});
	$('.nav-icon5').click(function(){
		$('.nav-icon1').removeClass('active')
		$('.nav-icon2').removeClass('active')
		$('.nav-icon3').removeClass('active')
		$('.nav-icon4').removeClass('active')
		$('.nav-icon5').addClass('active')
		$('.nav-icon6').removeClass('active')
	});
	$('.nav-icon6').click(function(){
		$('.nav-icon1').removeClass('active')
		$('.nav-icon2').removeClass('active')
		$('.nav-icon3').removeClass('active')
		$('.nav-icon4').removeClass('active')
		$('.nav-icon5').removeClass('active')
		$('.nav-icon6').addClass('active')
	});

    // tooltip js
	$('[data-toggle="tooltip"]').tooltip();
});